...
async function submitPassphrase({ valid }: { valid: boolean }): Promise<void> {
	...
	try {
		...
		const result = await res.json() as { id?: string; token?: string; expiresAt?: number | null; error?: string };
		if (!res.ok) {
			passphraseError.value = result.error ?? `エラー: ${res.status}`;
			return;
		}
		autoToken.value = result.token ?? null;
		autoTokenId.value = result.id ?? null;
		passphraseTokenExpiresAt.value = result.expiresAt ?? null;
		saveCachedToken(result.token ?? '', result.expiresAt ?? null, result.id ?? null);
